function [stochnumminLCC,Opts,e,D_0,Num_opts] = stochnumopp(n)
t = cputime; 
...
for i = 1:2^n  % the decimal index for a policy from 1 to 2^n
   ...
   stochnumLCCosts = stochnumLCC(c_a,c_r,r,mu,D_0); % compute the Life cycle cost for each given policy
   if (stochnumLCCosts < stochnumminLCC) % if the new policy is smaller than the previous one, 
       stochnumminLCC = stochnumLCCosts; % save it as the optimal(minimized) LCC
