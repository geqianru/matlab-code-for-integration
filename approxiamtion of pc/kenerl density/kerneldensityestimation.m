function [fout0,xout,u,ksinfo] = kerneldensityestimation(yData,varargin)
...
%                   you may want to choose a smaller value to reveal
...
%   KSDENSITY uses the transformation log((X-L)/(U-X)).  The 'bandwidth'
%   parameter and U outputs are on the scale of the transformed values.
%
...
okargs =   {'bandwidth' 'npoints'   'kernel'   'support' ...
            'weights'   'censoring' 'cutoff'   'function'   };
defaults = {[]          100         'normal'   'unbounded' ...
            1/n         false(n,1)  []         'pdf'};
[u,m,kernelname,support,weight,cens,cutoff,ftype,~,extra] = ...
    internal.stats.parseArgs(okargs, defaults, varargin{:});
if ~isempty(extra)
    u = internal.stats.parseArgs({'width'},{u},extra{:});
end
